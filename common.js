...
// ===== КИНЕТИЧЕСКИЕ КНОПКИ (ИСПРАВЛЕНЫ) =====
function initKineticButtons() {
    const buttonsToConvert = document.querySelectorAll(
        '.card-btn, .ai-card-btn, .btn-primary, .btn-outline, .hero-btn-primary, ' +
        '.hero-btn-outline, .section-more-btn, button[type="submit"], .header-telegram, #toTopBtn'
    );
    ...
    buttonsToConvert.forEach(oldBtn => {
        ...
        const segLeft = document.createElement('div'); segLeft.className = 'segment segment-left';
        const segCenter = document.createElement('div'); segCenter.className = 'segment segment-center';
        const segRight = document.createElement('div'); segRight.className = 'segment segment-right';
        const textSpan = document.createElement('div'); textSpan.className = 'btn-text';
        textSpan.textContent = btnText;
        for (let i = 0; i < 6; i++) { const spark = document.createElement('div'); spark.className = 'spark'; kineticBtn.appendChild(spark); }
        kineticBtn.appendChild(segLeft); kineticBtn.appendChild(segCenter); kineticBtn.appendChild(segRight); kineticBtn.appendChild(textSpan);
        wrapper.appendChild(kineticBtn);
        parent.replaceChild(wrapper, oldBtn);
        initKineticButtonEffects(kineticBtn, btnText);
        if (btnId === 'toTopBtn') {
            kineticBtn.addEventListener('click', function(e) { e.stopPropagation(); window.scrollTo({ top: 0, behavior: 'smooth' }); });
        }
        if (telegramLink) {
            kineticBtn.addEventListener('click', function(e) { e.stopPropagation(); window.open(telegramLink, '_blank'); });
        }
    });
}
...
function initKineticButtonEffects(btn, originalText) {
    ...
    const hoverColors = { left: '#D62976', centerTop: '#F56040', centerBottom: '#F56040', right: '#D62976' };
    ...
    function resetToWhite() {
        ...
        leftSeg.style.borderRight = 'none';
        rightSeg.style.borderLeft = 'none';
    }
    ...
    btn.addEventListener('mousemove', (e) => {
        ...
        if (Math.random() < 0.05) triggerSparks(1, e.clientX, e.clientY);
    });
    btn.addEventListener('mouseenter', () => { triggerSparks(3); setHoverColors(); });
    btn.addEventListener('mouseleave', () => {
        leftSeg.style.transform = ''; rightSeg.style.transform = ''; centerSeg.style.transform = '';
        resetToWhite(); targetRotateX = 0; targetRotateY = 0;
    });
    btn.addEventListener('click', (e) => { triggerSparks(8, e.clientX, e.clientY); btn.style.transform = `scale(0.97)`; setTimeout(() => { btn.style.transform = ''; }, 120); });
    resetToWhite();
}
...
function updateCalculator() {
    ...
    for (var i = 0; i < br.length; i++) timeHtml += '<div class="time-breakdown-item"><span>' + br[i].name + '</span><span>' + br[i].h + ' ч</span></div>';
    timeHtml += '</div>';
    var stackEl = document.getElementById('stackExplanation');
    if (stackEl) {
        if (h === 0) stackEl.innerHTML = '<h4>Рекомендуемый стек</h4><p>Выберите услуги для расчёта</p>';
        else stackEl.innerHTML = '<h4>Рекомендуемый стек</h4><p><strong>' + rec.rec + '</strong></p><h4>Преимущества</h4><p>' + rec.exp + '</p>' + timeHtml + '<p style="margin-top:12px; font-size:0.85rem;">Выбор технологий основан на потребностях вашего проекта.</p>';
    }
}
...
function initCalculator() {
    ...
    for (var i = 0; i < serviceOptions.length; i++) {
        ...
        o.addEventListener('click', function() {
            ...
            if (selected.has(s)) { selected.delete(s); this.classList.remove('selected'); }
            else { selected.add(s); this.classList.add('selected'); }
            updateCalculator();
        });
    }
    ...
    for (var i = 0; i < complexityOptions.length; i++) {
        ...
        o.addEventListener('click', function() {
            ...
            for (var j = 0; j < allOptions.length; j++) allOptions[j].classList.remove('selected');
...
function initCalculatorWithFallback() {
    ...
    if (staticTable && interactiveCalc) { staticTable.style.display = 'none'; interactiveCalc.style.display = 'block'; initCalculator(); }
    else if (interactiveCalc) { interactiveCalc.style.display = 'block'; initCalculator(); }
}
...
function initFaq() {
    ...
    if (faqGrid) {
        faqGrid.addEventListener('click', function(e) {
            ...
            for (var i = 0; i < allAnswersList.length; i++) allAnswersList[i].classList.remove('active');
            ...
            if (!isActive) { answer.classList.add('active'); button.classList.add('active'); }
        });
    }
}
...
// ===== ДОБАВЛЯЕМ ФУТЕР В АКТИВНУЮ ПАНЕЛЬ ПОРТФОЛИО =====
function addFooterToActivePanel() {
    ...
    footer.innerHTML = `<div class="footer-hours"><span>ДЛИТЕЛЬНОСТЬ</span><span>${hours}</span></div><div class="footer-price"><span>СТОИМОСТЬ</span><span>${price}</span></div>`;
    const content = activePanel.querySelector('.accordion-panel-content');
    if (content) content.after(footer);
    else activePanel.appendChild(footer);
}
...
function initPortfolioAccordion() {
    ...
    function closeAllPanels() { panels.forEach(panel => panel.classList.remove('active')); }
    ...
    panels.forEach(panel => {
        ...
        if (header) {
            ...
            header.addEventListener('click', function(e) { e.stopPropagation(); if (panel.classList.contains('active')) return; openPanel(panel); });
        }
        panel.addEventListener('click', function(e) {
            if (e.target.closest('a') || e.target.closest('button') || e.target.closest('.accordion-panel-content')) return;
            if (panel.classList.contains('active')) return;
            openPanel(panel);
        });
    });
    const activePanel = document.querySelector('.accordion-panel.active');
    if (!activePanel && panels.length > 0) { panels[0].classList.add('active'); addFooterToActivePanel(); }
    else if (activePanel) addFooterToActivePanel();
}

// ===== 3D ПАРАЛЛАКС ДЛЯ КАРТОЧЕК УСЛУГ =====
function initQuantumCards() {
    const cards = document.querySelectorAll('.services-grid .service-card');
    const prefersReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (prefersReducedMotion || cards.length === 0) return;
    cards.forEach(card => {
        const seg1 = card.querySelector('.seg-1'), seg2 = card.querySelector('.seg-2'), seg3 = card.querySelector('.seg-3'), seg4 = card.querySelector('.seg-4');
        const segments = [seg1, seg2, seg3, seg4];
        const content = card.querySelector('.card-content');
        if (!seg1 || !seg2 || !seg3 || !seg4) return;
        let targetRotateY = 0, currentRotateY = 0;
        let segTargets = { seg1: { x: 0, ry: 0 }, seg2: { x: 0 }, seg3: { x: 0 }, seg4: { x: 0, ry: 0 } };
        let segCurrent = { seg1: { x: 0, ry: 0 }, seg2: { x: 0 }, seg3: { x: 0 }, seg4: { x: 0, ry: 0 } };
        const MAX_X = 12, MAX_RY = 4;
        function limit(v, max) { return Math.min(max, Math.max(-max, v)); }
        function updateSegments() {
            segCurrent.seg1.x += (segTargets.seg1.x - segCurrent.seg1.x) * 0.2; segCurrent.seg1.ry += (segTargets.seg1.ry - segCurrent.seg1.ry) * 0.2;
            seg1.style.transform = `translateX(${segCurrent.seg1.x}px) rotateY(${segCurrent.seg1.ry}deg) translateZ(-5px)`;
            segCurrent.seg2.x += (segTargets.seg2.x - segCurrent.seg2.x) * 0.2; seg2.style.transform = `translateX(${segCurrent.seg2.x}px) translateZ(-5px)`;
            segCurrent.seg3.x += (segTargets.seg3.x - segCurrent.seg3.x) * 0.2; seg3.style.transform = `translateX(${segCurrent.seg3.x}px) translateZ(-5px)`;
            segCurrent.seg4.x += (segTargets.seg4.x - segCurrent.seg4.x) * 0.2; segCurrent.seg4.ry += (segTargets.seg4.ry - segCurrent.seg4.ry) * 0.2;
        ...
        card.addEventListener('mousemove', (e) => {
            ...
            card.style.setProperty('--x', px + '%'); card.style.setProperty('--y', py + '%');
            const intensity = Math.min(1, Math.abs(relX));
            const borderGlow = `rgba(255, 255, 255, ${0.15 + intensity * 0.4})`;
            segments.forEach(seg => { if (seg) seg.style.borderColor = borderGlow; });
            segTargets.seg1.x = limit(relX * -12, MAX_X); segTargets.seg1.ry = limit(relX * -4, MAX_RY);
            segTargets.seg2.x = limit(relX * -5, MAX_X); segTargets.seg3.x = limit(relX * 5, MAX_X);
            segTargets.seg4.x = limit(relX * 12, MAX_X); segTargets.seg4.ry = limit(relX * 4, MAX_RY);
            if (content) content.style.transform = `translateZ(25px)`;
        });
        card.addEventListener('mouseleave', () => {
            targetRotateY = 0;
            segTargets = { seg1: { x: 0, ry: 0 }, seg2: { x: 0 }, seg3: { x: 0 }, seg4: { x: 0, ry: 0 } };
            segments.forEach(seg => { if (seg) seg.style.borderColor = ''; });
            if (content) content.style.transform = 'translateZ(25px)';
        });
    });
}
...
function initCaseAccordion() {
    ...
    if (!sparkContainer) {
        ...
        sparkContainer.style.position = 'fixed'; sparkContainer.style.top = '0'; sparkContainer.style.left = '0';
        sparkContainer.style.width = '100%'; sparkContainer.style.height = '100%';
        sparkContainer.style.pointerEvents = 'none'; sparkContainer.style.zIndex = '9999';
        document.body.appendChild(sparkContainer);
    }
    const sparks = [];
    const SPARKS_COUNT = 30;
    for (let i = 0; i < SPARKS_COUNT; i++) { const s = document.createElement('div'); s.className = 'spark'; sparkContainer.appendChild(s); sparks.push(s); }
    function burstSparks(x, y) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            spark.style.left = (x - 2) + 'px'; spark.style.top = (y - 2) + 'px';
            spark.style.background = colors[Math.floor(Math.random() * colors.length)];
            spark.style.animation = 'none'; spark.offsetHeight;
    ...
    function openCase(selectedItem) {
        ...
        items.forEach(item => { if (item !== selectedItem && item.classList.contains('active')) item.classList.remove('active'); });
        if (!isActive) {
        ...
        } else { selectedItem.classList.remove('active'); }
    }
    items.forEach(item => {
        const lid = item.querySelector('.case-lid');
        if (lid) lid.addEventListener('click', (e) => { e.preventDefault(); openCase(item); });
    });
}
...
function loadComponent(elementId, filePath, callback) {
    ...
    fetch(filePath)
        .then(response => { if (!response.ok) throw new Error('Failed to load ' + filePath); return response.text(); })
        .then(data => { placeholder.innerHTML = data; if (callback) callback(); })
        .catch(error => { console.error('Ошибка загрузки компонента:', error); placeholder.innerHTML = '<div style="padding:20px;text-align:center;">Ошибка загрузки</div>'; });
}
