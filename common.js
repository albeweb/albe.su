...
(function() {
    ...
    // ============================================// ============================================
// common.js — ПОЛНАЯ ИСПРАВЛЕННАЯ ВЕРСИЯ С ЛЕНИВОЙ ЗАГРУЗКОЙ БЛОКОВ
// ============================================
...
(function() {
    ...
    // ============================================
    // ============================================
    // НОВАЯ ФУНКЦИЯ: ЛЕНИВАЯ ЗАГРУЗКА БЛОКОВ ПРИ СКРОЛЛЕ
    // ============================================
    // ============================================
    // ============================================

    function initLazyBlocks() {
        const lazyBlocks = document.querySelectorAll('.lazy-block-container');
        
        if (lazyBlocks.length === 0) return;
        
        // Создаём наблюдатель для каждого блока
            ...
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    if (!isLoaded) {
                        ...
                        container.classList.add('lazy-block-visible');
                        
                        // Инициализируем изображения внутри блока (data-src -> src)
                        const lazyImages = container.querySelectorAll('img[data-src], img.lazy-image');
                        ...
                        // Инициализируем аккордеон портфолио, если это блок portfolio
                        const blockName = container.getAttribute('data-lazy-block');
                        if (blockName === 'portfolio') {
                            setTimeout(() => {
                                if (typeof initPortfolioAccordion === 'function') initPortfolioAccordion();
                            }, 50);
                        }
                        
                        // Инициализируем калькулятор, если это блок calculator
                        if (blockName === 'calculator') {
                            setTimeout(() => {
                                if (typeof initCalculatorWithFallback === 'function') initCalculatorWithFallback();
                            }, 50);
                        }
                        
                        // Инициализируем FAQ аккордеон, если это блок faq
                        if (blockName === 'faq') {
                            setTimeout(() => {
                                if (typeof initCaseAccordion === 'function') initCaseAccordion();
                            }, 50);
                        }
                        
                        // Инициализируем видео-плейсхолдеры
                        if (blockName === 'ai-reklama') {
                            setTimeout(() => {
                                if (typeof initLazyKinescope === 'function') initLazyKinescope();
                            }, 50);
                        }
                        
                        console.log(`Блок "${blockName}" загружен при скролле`);
                    }
                }
            });
        }, { rootMargin: '200px', threshold: 0.05 }); // Начинаем загрузку за 200px до появления
        
        // Наблюдаем за каждым блоком
        lazyBlocks.forEach(block => blockObserver.observe(block));
    }
    
    // Добавляем стили для анимации появления блоков
    function addLazyBlockStyles() {
        if (document.getElementById('lazy-block-styles')) return;
        const style = document.createElement('style');
        style.id = 'lazy-block-styles';
        style.textContent = `
            ...
            .lazy-block-container.lazy-block-visible {
                opacity: 1;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Добавляем стили для ленивых блоков
        addLazyBlockStyles();
        ...
        // Запускаем ленивую загрузку блоков (ОСНОВНАЯ ФИШКА!)
        initLazyBlocks();
